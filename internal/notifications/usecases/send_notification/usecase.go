...
func NewSendNotificationUseCase(tenantService *tenants.TenantService, providerService *providers.ProviderService, templateService *templates.TemplateService, notificationRepository repository.NotificationRepository) *SendNotificationUseCase {
...
func (u *SendNotificationUseCase) Execute(ctx context.Context, req SendNotificationRequest) SendNotificationResponse {
	...
	if err != nil {
		return SendNotificationResponse{
			ID:     notification.ID,
			Status: "failed",
			Error:  "notification content creation failed: " + err.Error(), // Detailed error message
		}
	}
	...
	//update notification repo saying it was send sucessfully or Failure Message
	//In web UI notification that are send to the client are send thorugh websocket microservice which hadles clients websocket connection and send whatever communication to and fro thorough a queue within connected clients and other microservices please put place holder for now will add later
